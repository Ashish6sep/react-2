...
import { CRYPTO_ENCRYPTION, AJAX_REQUEST, AJAX_ACCOUNT_KIT_REQUEST, CURRENCY_FORMAT, CART_TOTAL_CURRENCY_FORMAT, COUNT_SUBSCRIPTION, SET_STORAGE, GET_STORAGE, REMOVE_STORAGE, CHECK_STORAGE, DESTROY_CART, ITEM_COUNT, MEAL_COUNT, NEXT_MONTH, NEXT_WEEK } from "../../Constants/AppConstants";
...
class CheckOut extends PureComponent {
        ...
        if (this.state.taxStatus == 1) {
            ...
            if (address_1 != '' && postcode != '' && city != '' && state != '' && country != '') {
                ...
                let taxData = {
                    ...
                    meals: this.state.meals, // set meals
                ...
                AJAX_REQUEST("POST", "order/getTax", taxData).then(results => {
                    if (parseInt(results.response.code) === 1000 && results.response.data != '') {
                        this.setState({
                            tax_amount: (results.response.data.tax_amount) ? results.response.data.tax_amount : 0,
                            tax_info: results.response.data.tax_info,
                            subscription_tax_amount: (results.response.data.subscription_tax_amount) ? results.response.data.subscription_tax_amount : 0,
                            subscription_tax_info: results.response.data.subscription_tax_info,
                            meal_tax_amount: (results.response.data.meal_tax_amount) ? results.response.data.meal_tax_amount : 0,
                            meal_tax_info: results.response.data.meal_tax_info,
                            subscription_meal_tax_amount: (results.response.data.subscription_meal_tax_amount) ? results.response.data.subscription_meal_tax_amount : 0,
                            subscription_meal_tax_info: results.response.data.subscription_meal_tax_info,
                        });
                    }
                });
            }
    ...
    getCart = () => {
        ...
        if (this.props.meals.length > 0 && this.props.meals[0]) {
            this.setState({ meals: this.props.meals[0] });
        } else {
            this.setState({ meals: [] });
        }
        ...
        if (couponCode == '' || couponCode == null) {
            ...
            if (exCouponList.length > 0) {
                exCouponList.forEach(function (exCoupon, key) {
                    if (exCoupon.coupon_code === couponCode) {
                        couponExists = true;
                    }
                });
            }
                                                                    ...
                                                                    show_coupon ?
                                                                        <Fragment>
                                                                            <div className="coupon checkout_coupon checkout_coupon_apply">
                                                                                ...
                                                                                <input onClick={this.applyCoupon} type="submit" className="button cus_button" value={coupon_code_button_loading ? "Please Wait..." : "Apply Coupon"} />
                                                                            </div>
                                                                        </Fragment>
                                                                        : null
                                                                }
                                                                ...
                                                                <div className="cart_totals review-order-table-wrapper">
                                                                    ...
                                                                    <table cellSpacing="0" className="shop_table checkout_table ">
                                                                        ...
                                                                        <tbody>
                                                                                ...
                                                                                (this.state.items.length <= 0) ? null :
                                                                                    this.state.items.map(function (item, key) {
                                                                                        ...
                                                                                        if (item.subscription == 'yes') {
                                                                                            cartSubTotal = Number(cartSubTotal) + Number(item.quantity * item.cart_discount_price);
                                                                                            recurringSubTotal = Number(recurringSubTotal) + Number(item.quantity * item.cart_discount_price);
                                                                                        } else {
                                                                                            cartSubTotal = Number(cartSubTotal) + Number(item.quantity * item.cart_sale_price);
                                                                                        }
                                                                                        this.setState({
                                                                            ...
                                                                            {/* Meal start  */}
                                                                            {
                                                                                (this.state.meals != null && this.state.meals != '') ?
                                                                                    <Fragment>
                                                                                        {
                                                                                            (this.state.meals.subscription == 'yes') ?
                                                                                                this.setState({
                                                                                                    cartSubTotal: Number(cartSubTotal) + Number(this.state.meals.discount_price * this.state.meals.quantity),
                                                                                                })
                                                                                                :
                                                                                                this.setState({
                                                                                                    cartSubTotal: Number(cartSubTotal) + Number(this.state.meals.sale_price * this.state.meals.quantity),
                                                                                                })
                                                                                        }
                                                                                        <tr className="cart_item">
                                                                                            <td className="product-name">
                                                                                                <span className="checkout_product_name"> {this.state.meals.product_name} </span>
                                                                                                <span className="product-quantity checkout-product-quantity">Quantity: {this.state.meals.quantity}</span>
                                                                                                {
                                                                                                    (this.state.meals.items.length <= 0) ? null :
                                                                                                        this.state.meals.items.map(function (item, key) {
                                                                                                            return (
                                                                                                                <Fragment key={Math.random()}>
                                                                                                                    <span style={{ fontSize: "12px" }}><i className="fa fa-circle" aria-hidden="true"></i> {item.meal_name} x {item.meal_quantity}</span><br />
                                                                                                                </Fragment>
                                                                                                            )
                                                                                                        }.bind(this))
                                                                                                }
                                                                                                {
                                                                                                    (this.state.meals.premium == "yes") ?
                                                                                                        <p>Premium (+{this.state.meals.premium_amount})</p>
                                                                                                        : null
                                                                                                }
                                                                                                {
                                                                                                    (this.state.meals.subscription == "yes") ?
                                                                                                        <dl className="variation">
                                                                                                            ...
                                                                                                            <dd className="variation-Every1Months"><strong> {CURRENCY_FORMAT(this.state.meals.discount_price)} each</strong></dd>
                                                                                                        </dl>
                                                                                                        : null
                                                                                                }
                                                                                            </td>
                                                                                            <td className="product-total">
                                                                                                <span className="subscription-price"><span className="woocommerce-Price-amount amount"><span className="woocommerce-Price-currencySymbol">
                                                                                                    {
                                                                                                        (this.state.meals.subscription == "yes") ?
                                                                                                            CURRENCY_FORMAT(this.state.meals.quantity * this.state.meals.discount_price)
                                                                                                            :
                                                                                                            CURRENCY_FORMAT(this.state.meals.quantity * this.state.meals.sale_price)
                                                                                                    }
                                                                                                </span></span></span>
                                                                                            </td>
                                                                                        </tr>
                                                                            ...
                                                                            <tr className="cart-subtotal">
                                                                                ...
                                                                                <td data-title="Subtotal">
                                                                                    <span className="woocommerce-Price-amount amount">
                                                                                        <span className="woocommerce-Price-currencySymbol"> <strong>{CURRENCY_FORMAT(this.state.cartSubTotal)} </strong></span>
                                                                                    </span>
                                                                                </td>
                                                                            </tr>
                                                                            {
                                                                                ...
                                                                                (this.state.coupons.length <= 0) ?
                                                                                    this.setState({ couponTotal: 0 })
                                                                                    :
                                                                                    this.state.coupons.map(function (coupon, key) {

                                                                                        couponTotal = Number(couponTotal) + Number(coupon.discount_amount);
                                                                                        this.setState({ couponTotal: couponTotal })
                                                                                        ...
                                                                                        if (coupon.free_shipping == 1 && coupon.coupon_type == "meal") {

                                                                                            if (GET_STORAGE("meals")) {
                                                                                                let meals = JSON.parse(GET_STORAGE("meals"));
                                                                                                meals.shipping_cost = 0;
                                                                                            ...
                                                                                            if (GET_STORAGE("meals")) {
                                                                                                let meals = JSON.parse(GET_STORAGE("meals"));
                                                                                                meals.shipping_cost = meals.subscription_shipping_cost;
                                                                                                SET_STORAGE("meals", JSON.stringify(meals));
                                                                                            }
                                                                                            ...
                                                                                            <td data-title="Meal Shipping Cost">
                                                                                                <span className="woocommerce-Price-amount amount">
                                                                                                    <span className="woocommerce-Price-currencySymbol">
                                                                                                        {
                                                                                                            (this.state.meals.shipping_cost > 0 && this.state.taxFree == 0) ?
                                                                                    ...
                                                                                    <strong>
                                                                                        <span className="woocommerce-Price-amount amount">
                                                                                            <span className="woocommerce-Price-currencySymbol">
                                                                                                {
                                                                                                    (this.state.items.length > 0) ?
                                                                                                        (this.state.meals != null && this.state.meals != '' && this.state.taxFree == 0) ?
                                                                                                            this.setState({
                                                                                                                cartTotal: Number(this.state.cartSubTotal - this.state.couponTotal) + Number(this.state.tax_amount) + Number(this.state.meal_tax_amount) + Number(this.state.cartShippingCost) + Number(this.state.meals.shipping_cost)
                                                                                                            })
                                                                                                            :
                                                                                                            this.setState({
                                                                                                                cartTotal: Number(this.state.cartSubTotal - this.state.couponTotal) + Number(this.state.tax_amount) + Number(this.state.cartShippingCost)
                                                                                                            })
                                                                                                        :
                                                                                                        (this.state.meals != null && this.state.meals != '' && this.state.taxFree == 0) ?
                                                                                                            this.setState({
                                                                                                                cartTotal: Number(this.state.cartSubTotal - this.state.couponTotal) + Number(this.state.meal_tax_amount) + Number(this.state.meals.shipping_cost)
                                                                                                            })
                                                                                                            : this.setState({
                                                                                                                cartTotal: Number(this.state.cartSubTotal - this.state.couponTotal) + Number(this.state.meal_tax_amount)
                                                                                                            })
                                                                                                }
                                                                                    ...
                                                                                    <Fragment>
                                                                                        ...
                                                                                        <tr className="cart-subtotal recurring-total">
                                                                                            ...
                                                                                            <td className="subtotal">
                                                                                                <span className="woocommerce-Price-amount amount"><span className="woocommerce-Price-currencySymbol"> {CURRENCY_FORMAT(this.state.recurringSubTotal)} </span></span> / month
                                                                                    </td>
                                                                                        </tr>
                                                                                        <tr className="woocommerce-shipping-totals shipping">
                                                                                            <td>SHIPPING COST</td>
                                                                                            ...
                                                                                            <td className="recurring_total">
                                                                                                <div className="total_per_month">
                                                                                                    <span><span className="woocommerce-Price-currencySymbol">
                                                                                                        {
                                                                                                            CURRENCY_FORMAT(Number(this.state.recurringSubTotal) + Number(this.state.recurringCartShippingCost) + Number(this.state.subscription_tax_amount))
                                                                                                        }
                                                                            ...
                                                                            {
                                                                                (this.state.meals.subscription != 'yes') ? null :
                                                                                    <Fragment>
                                                                                        <tr>
                                                                                        ...
                                                                                        <tr className="cart-subtotal recurring-total">
                                                                                            <td rowSpan="1">Subtotal</td>
                                                                                            <td className="subtotal">
                                                                                                <span className="woocommerce-Price-amount amount"><span className="woocommerce-Price-currencySymbol"> {CURRENCY_FORMAT(this.state.meals.discount_price * this.state.meals.quantity)} </span></span> / week
                                                                                            </td>
                                                                                        </tr>
                                                                                        <tr className="woocommerce-shipping-totals shipping">
                                                                                            <td>Meal SHIPPING COST</td>
                                                                                            <td data-title="Meal Shipping Cost">
                                                                                                <span className="woocommerce-Price-amount amount">
                                                                                                    {
                                                                                                        (this.state.meals.subscription_shipping_cost > 0) ?
                                                                                                            CURRENCY_FORMAT(this.state.meals.subscription_shipping_cost)
                                                                                                            : "Free Shipping"
                                                                                                    }
                                                                                                </span>
                                                                                            </td>
                                                                                        ...
                                                                                        <tr className="order-total">
                                                                                            ...
                                                                                            <td className="recurring_total">
                                                                                                <div className="total_per_month">
                                                                                                    <span><span className="woocommerce-Price-currencySymbol">
                                                                                                        {
                                                                                                            CURRENCY_FORMAT(Number(this.state.meals.discount_price * this.state.meals.quantity) + Number(this.state.meals.subscription_shipping_cost) + Number(this.state.subscription_meal_tax_amount))
                                                                                                        }
                                                                                                    </span></span><span className="per_month"> / week</span>
                                                                                                </div>
                                                                                                <div className="first-payment-date">
                                                                                                    <small>First renewal: {NEXT_WEEK()}</small>
                                                                                                </div>
                                                                                            </td>
                                                                                        </tr>
                                                                                    </Fragment>
                                                                    ...
                                                                    (
                                                                        ...
                                                                        <Fragment>
                                                                            <div className="checkout-payment-card-opt">
                                                                                <label htmlFor="payment_method_icanpay"> iCanPay  </label>
                                                                                <img src={require("../../Assets/images/card.png")} alt="iCanPay" />
                                                                            </div>
                                                                        </Fragment>
                                                                }
